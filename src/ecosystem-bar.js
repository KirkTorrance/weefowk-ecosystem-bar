(function () {
  ...
  // VoteWiser first, Wee Fowk second, Organising third (soon)
  var NETWORK_SITES = [
    { name: 'VoteWiser',  url: 'https://votewiser.scot',            description: 'Electoral Intelligence', icon: 'V' },
    { name: 'Wee Fowk',   url: 'https://weefowk.org',              description: 'The Network',            icon: 'W' },
    { name: 'Organising', url: 'https://organisingforscotland.com', description: 'Organising Engine',      icon: 'O', comingSoon: true }
  ];
  ...
  var css = [
    ...
    '#wfbar-hex{color:'+GOLD+';font-size:17px;line-height:1;flex-shrink:0;font-weight:700}',
    ...
    // Dropdown panel
    '#wfbar-dd{position:absolute;top:calc(100% + 4px);left:0;background:'+BLUE_DARK+';border:1px solid '+WHITE_15+';border-radius:8px;min-width:250px;padding:6px;z-index:99999;opacity:0;visibility:hidden;transform:translateY(-6px);transition:opacity .15s,transform .15s,visibility .15s;pointer-events:none;box-shadow:0 16px 40px rgba(0,0,0,.45)}',
    ...
    // Dropdown rows
    '.wfdd-row{display:flex;align-items:center;gap:12px;padding:9px 12px;border-radius:5px;text-decoration:none;transition:background .12s;cursor:pointer}',
    ...
    // Icon box – single gold letter, clean and consistent
    '.wfdd-icon{width:32px;height:32px;border-radius:6px;background:rgba(255,255,255,.08);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:13px;font-weight:700;color:'+GOLD+';letter-spacing:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}',
    '.wfdd-row.wf-cur .wfdd-icon{background:'+GOLD_20+'}',
    '.wfdd-row.wf-soon .wfdd-icon{opacity:.3}',

    // Row text
    '.wfdd-text{display:flex;flex-direction:column;gap:2px;min-width:0}',
    ...
    '.wfdd-desc{font-size:11px;color:rgba(255,255,255,.4);letter-spacing:.1px}',

    // Soon badge
    '.wfdd-soon-tag{font-size:9px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:'+GOLD+';opacity:.6;margin-left:auto;padding:2px 7px;border:1px solid rgba(240,192,64,.3);border-radius:3px;flex-shrink:0}',
    ...
    '#wfbar-bell-badge{position:absolute;top:4px;right:4px;width:7px;height:7px;border-radius:50%;background:'+RED_SOFT+';border:1.5px solid '+BLUE+';display:none}',
    ...
    // Buttons – ghost style (logged-in sign out)
    ...
    // Notification strip – sits immediately below the bar
    ...
    '#wfbar-strip-msg{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;text-align:center}',
    '#wfbar-strip-close{background:none;border:none;cursor:pointer;font-size:18px;line-height:1;opacity:.5;padding:0 0 0 8px;flex-shrink:0;transition:opacity .15s;font-family:inherit}',
    '#wfbar-strip-close:hover{opacity:1}',

    // Spacer pushes page content below bar + strip
  ...
  // Admin dashboard writes to network_config table.
  // Bar reads it on every page load. Change the DB, 
  // every site on the network updates within seconds.
  // ============================================================
  ...
  function build(cfg) {
    ...
    var showStrip    = notifActive && notifMessage.length > 0;
    var totalOffset  = BAR_HEIGHT + (showStrip ? STRIP_HEIGHT : 0);
    ...
    tw.id = 'wfbar-tag-wrap';
    var tg = document.createElement('span');
    tg.id = 'wfbar-tag';
    tg.innerHTML = '<span id="wfbar-tag-full">'+TAGLINE_FULL+'</span>'
                 + '<span id="wfbar-tag-mobile">'+TAGLINE_MOBILE+'</span>';
    tw.appendChild(tg);
    left.appendChild(tw);
    ...
    var ddHdr = document.createElement('div');
    ddHdr.id = 'wfbar-dd-hdr';
    ...
    NETWORK_SITES.forEach(function(s) {
      ...
      row.className = 'wfdd-row'
        + (isCurrent   ? ' wf-cur'  : '')
        + (s.comingSoon? ' wf-soon' : '');

      var iconEl = document.createElement('div');
      iconEl.className = 'wfdd-icon';
      iconEl.textContent = s.icon;

      var textEl = document.createElement('div');
      textEl.className = 'wfdd-text';
      textEl.innerHTML = '<span class="wfdd-name">'+s.name+'</span>'
                       + '<span class="wfdd-desc">'+s.description+'</span>';
    ...
    // Toggle dropdown – mousedown beats React synthetic click
    ...
    bellBadge.id = 'wfbar-bell-badge';
    if (showStrip) bellBadge.classList.add('wf-show');
    ...
    if (user) {
      ...
      soBtn.className = 'wfbtn';
      soBtn.setAttribute('type', 'button');
    ...
    var stripDot = document.createElement('span');
    stripDot.id = 'wfbar-strip-dot';
    stripDot.style.background = palette.dot;

    var stripMsg = document.createElement('span');
    stripMsg.id = 'wfbar-strip-msg';
    stripMsg.textContent = notifMessage;

    var stripClose = document.createElement('button');
    stripClose.id = 'wfbar-strip-close';
    stripClose.setAttribute('type', 'button');
    stripClose.innerHTML = '&times;';
    stripClose.style.color = palette.text;
    ...
    if (showStrip) strip.classList.add('wf-strip-show');

    // ── SPACER ────────────────────────────────────────────────
    var spacer = document.createElement('div');
    spacer.id = 'wfbar-spacer';
